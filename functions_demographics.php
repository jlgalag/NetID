			   ...
			   // Check connection
				if (!mysqli_connect_errno($conn)){
					...
					while($row2 = mysqli_fetch_array($degreeprograms)){
						...
						$sr = ldap_search($ldapconn, "ou=student,ou=people,".$ldapconfig['basedn'], "(&(ou=".$row2['name'].")(gidnumber=".$row2['gidnumber']."))");
			   ...
			   // Check connection
				if (!mysqli_connect_errno($conn)){
					...
					while($row2 = mysqli_fetch_array($college)){
						echo '<tr>';
						echo '<th>'.$row2['name'].'</th>';
						$sr = ldap_search($ldapconn, "ou=student,ou=people,".$ldapconfig['basedn'], "(gidnumber=".$row2['gidnumber'].")");
						echo '<td>'.ldap_count_entries($ldapconn, $sr).'</td>';
						echo '</tr>';
					}
					  echo "</tbody>";
