	...
	function encrypt($string){
     ...
     function searchstudent($filter){
	   global $ldapconn;
 	   ...
 	   //$editmail = array("mail" => array($newmail));
       $res = ldap_search($ldapconn, "ou=student,ou=people,dc=intranet,dc=uplb,dc=edu,dc=ph", $filter);
       $entries = ldap_get_entries($ldapconn, $res);
	   if($res){  
	    //echo "<h4>".$filter."</h4>";
		if(!($entries["count"] > 0))
	 ...
	 function searchemployee($filter){
	   global $ldapconn;
 	   ...
 	   //$editmail = array("mail" => array($newmail));
       $res = ldap_search($ldapconn, "ou=employee,ou=people,dc=intranet,dc=uplb,dc=edu,dc=ph", $filter);
       $entries = ldap_get_entries($ldapconn, $res);
	   if($res){  
	     // echo "<h4>".$filter."</h4>";
		  if(!($entries["count"] > 0))
	 ...
	 function searchuid($uid){
	   global $ldapconn;
 	   ...
 	   //$editmail = array("mail" => array($newmail));
       $res = ldap_search($ldapconn, "ou=people,dc=intranet,dc=uplb,dc=edu,dc=ph", "(uid=*".$uid."*)");
       $entries = ldap_get_entries($ldapconn, $res);
