<?php
   ...
   function viewstudentlist($name){
	   ...
	   $res = ldap_search($ldapconn, "ou=student,ou=people,dc=intranet,dc=uplb,dc=edu,dc=ph", "(ou=".$name.")");
    ...
    function viewemployeelist($gidnumber, $name){
	   ...
	   $res = ldap_search($ldapconn, "ou=employee,ou=people,dc=intranet,dc=uplb,dc=edu,dc=ph", "(gidnumber=".$gidnumber.")");
        ldap_sort($ldapconn, $res, "cn");
	   $entries = ldap_get_entries($ldapconn, $res);
	   echo "<h4>".$name."</h4>";
